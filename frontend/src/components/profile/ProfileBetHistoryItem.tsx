    ...
    <article className="flex w-full items-center gap-3 rounded-xl bg-[#141829] px-3.5 py-3">
      <div
        className={`flex size-9 shrink-0 items-center justify-center rounded-[10px] ${
          win ? 'bg-[#22c55e]/20 text-[#22c55e]' : 'bg-[#ef4444]/20 text-[#ef4444]'
        }`}
      >
        {win ? <Trophy className="size-4" strokeWidth={2.25} aria-hidden /> : <CircleX className="size-4" strokeWidth={2.25} aria-hidden />}
      </div>
      ...
      <div className="flex shrink-0 flex-col items-end gap-px">
        <p className={`font-[family-name:var(--font-inter)] text-sm font-bold ${win ? 'text-[#22c55e]' : 'text-[#ef4444]'}`}>
          {amountLabel}
        </p>
        <p className={`font-[family-name:var(--font-inter)] text-[9px] font-semibold ${win ? 'text-[#22c55e]' : 'text-[#ef4444]'}`}>
          {resultLabel}
        </p>
      </div>
    </article>
